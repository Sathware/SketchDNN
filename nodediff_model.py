...
class GD3PM(nn.Module):
  ...
  def denoise(self, nodes, axes = None):
    ...
    #   if t % stepsize == 0:
    #     SketchDataset.render_graph(ToNaive(nodes[0,...,1:]).cpu(), torch.zeros(size = (24, 24, 17)).cpu(), axes[0, j])
    #     SketchDataset.render_graph(ToNaive(denoised_nodes[0,...,1:]).cpu(), torch.zeros(size = (24, 24, 17)).cpu(), axes[1, j])
    #     j = j + 1
    
    # SketchDataset.render_graph(ToNaive(nodes[0,...,1:]).cpu(), torch.zeros(size = (24, 24, 17)).cpu(), axes[0, num_images - 1])
    # SketchDataset.render_graph(ToNaive(denoised_nodes[0,...,1:]).cpu(), torch.zeros(size = (24, 24, 17)).cpu(), axes[1, num_images - 1])
...
class DiffusionModel(nn.Module):
    def __init__(self, node_dim, node_hidden_dim, cond_hidden_dim, num_heads, num_tf_layers, num_checkpoints, max_timestep, device: device):
        ...
        # self.pos_embedder = nn.Embedding(num_embeddings = 16, embedding_dim = cond_hidden_dim, device = device)
        ...
        #     nn.Linear(in_features = node_hidden_dim, out_features = 4 * node_hidden_dim, device = device),
        #     nn.SiLU(),
        #     nn.Linear(in_features = 4 * node_hidden_dim, out_features = 6, device = device)
        # )
        # self.lin_out_params = nn.Sequential(
        #     nn.Linear(in_features = node_hidden_dim, out_features = 4 * node_hidden_dim, device = device),
        #     nn.SiLU(),
        ...
        #     nn.Linear(in_features = node_hidden_dim, out_features = 4 * node_hidden_dim, device = device),
        #     nn.SiLU(),
        #     nn.Linear(in_features = 4 * node_hidden_dim, out_features = 5, device = device)
        # )
        # self.lin_out_bool = nn.Sequential(
        #     nn.Linear(in_features = node_hidden_dim, out_features = 4 * node_hidden_dim, device = device),
        #     nn.SiLU(),
        #     nn.Linear(in_features = 4 * node_hidden_dim, out_features = 1, device = device)
        # )
    ...
    def forward(self, nodes : Tensor, timestep : Tensor):
        ...
        conds = F.silu(self.time_embedder(timestep)).unsqueeze(1) # F.silu(self.time_embedder(timestep).unsqueeze(1) + self.pos_embedder.weight.unsqueeze(0)) # self.mlp_in_conds(conds)     # shape: (batch_size, num_nodes, cond_hidden_dim)
        ...
        # nodes = torch.cat([torch.zeros_like(nodes[...,[0]]), self.lin_out_types(nodes), self.lin_out_params(nodes)], dim = -1) # shape: (batch_size, num_nodes, node_dim)
        nodes = self.mlp_out(nodes) # torch.cat([torch.zeros_like(nodes[...,[0]]), self.lin_out_bool(nodes), self.lin_out_types(nodes), self.lin_out_params(nodes)], dim = -1) # shape: (batch_size, num_nodes, node_dim)
