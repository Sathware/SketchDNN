...
class TransformerLayer(nn.Module):
    def __init__(self, node_dim: int, edge_dim : int, cond_dim: int, num_heads: int, device: device):
        ...
        self.norm_edge_in = Normalization(node_dim = edge_dim, device = device, affine = True)
        ...
        self.norm_edge_attn = Normalization(node_dim = edge_dim, device = device, affine = True)
        ...
        self.mlp_edges = nn.Sequential(
            ...
            nn.GELU(approximate = "tanh"), # nn.SiLU(),
            nn.Linear(in_features = edge_dim, out_features = edge_dim, device = device),
            # nn.SiLU()
        )
    ...
    def forward(self, nodes : Tensor, edges : Tensor, conds : Tensor) -> Tensor:
        ...
        # Attention
        deltanodes, deltaedges = self.attention_heads(self.norm_node_in(nodes) * (1 + mul_in_node) + add_in_node,
                                                      self.norm_edge_in(edges)) # * (1 + mul_in_edge) + add_in_edge
        ...
        nodes = self.mlp_nodes(self.norm_node_attn(nodes) * (1 + mul_attn_node) + add_attn_node) + nodes

        # bias = self.lin_cond_node_edge(norm_nodes.detach().unsqueeze(1) + norm_nodes.detach().unsqueeze(2))
        edges = self.mlp_edges(self.norm_edge_attn(edges)) + edges # * (1 + mul_attn_edge) + add_attn_edge
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, nodes : Tensor, edges : Tensor) -> Tensor:
        ...
        weighted_values = torch.dropout(attn, 0.1, self.training).sum(dim = -1).softmax(dim = -1) @ values
        ...
        return self.lin_nodes_out(weighted_values), self.lin_edges_out(edges)
